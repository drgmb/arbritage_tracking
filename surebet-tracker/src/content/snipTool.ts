...
interface SnipState {
  ...
  instructions: HTMLDivElement | null;
  actionsPanel: HTMLDivElement | null;
  legId: number | null;
}

const state: SnipState = {
  isActive: false,
  isSelecting: false,
  startX: 0,
  startY: 0,
...
function createSelectionBox(x: number, y: number): void {
...
function updateSelectionBox(endX: number, endY: number): void {
...
function showPreview(selection: SelectionBox): void {
  if (!state.selectionBox) return;
...
async function confirmSelection(selection: SelectionBox): Promise<void> {
  cleanup();
...
function activate(legId: number): void {
